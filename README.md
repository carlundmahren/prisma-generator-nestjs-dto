...
```sh
yarn add --dev @carlundmahren/prisma-generator-nestjs-dto
```
...
- [`forceIdOnConnect`]: (default: `"false"`) - Force `id` field on `ConnectDTO` to be required.
...
- `@DtoRelationCanUpdateOnUpdate` - adds [update](https://www.prisma.io/docs/concepts/components/prisma-client/relation-queries#connect-an-existing-record) option on a relation field in the generated `UpdateDTO` - useful when you want/need to update related model instances
